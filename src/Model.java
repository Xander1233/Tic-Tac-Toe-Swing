public class Model {
	...
	private static final int[][] WIN_POSSIBILITIES = { { 0, 1, 2 }, { 0, 4, 8 }, { 2, 4, 6 }, { 6, 7, 8 }, { 3, 4, 5 }, { 0, 3, 6 }, { 1, 4, 7 }, { 2, 5, 8 } };
	...
	public void checkWin(int player) {
		...
		for (int[] winPo : WIN_POSSIBILITIES) {
			won = true;
	...
	public boolean fieldEmpty(int pos) {
		return !((field & (1 << pos)) > 0 || (field & (1 << (pos + 9))) > 0);
	}

	public void reset() {
		this.field = 0b0;
		this.winFlag = WinStatusFlag.NONE;
		this.fieldsEmpty = 9;
	}
